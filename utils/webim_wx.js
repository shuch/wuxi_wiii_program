...
(function (webim) {
    ...
    var tool = new function () {
        ...
        this.xssFilter = function (val) {
            ...
            val = val.replace(/[<]/g, "&lt;");
            val = val.replace(/[>]/g, "&gt;");
            // val = val.replace(/"/g, "&quot;"); // 为了和IOS 安卓等机器 发送视频邀请，特别去除该行，注意
            //val = val.replace(/'/g, "&#039;");
            return val;
        };
    ...
    var MsgStore = new function () {
        ...
        this.addMsg = function (msg) {
            ...
            // sess._impl.msgs.push(msg);
            if (msg.time > sess._impl.time){
                sess._impl.time = msg.time;
            }
            if (msg.seq > sess._impl.curMaxMsgSeq){
                sess._impl.curMaxMsgSeq = msg.seq;
            }
            if (!msg.isSend && !sess._impl.isAutoRead) {
                sess._impl.unread++;
            }
            return true;
        };
