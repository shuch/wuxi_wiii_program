...
function getC2CHistoryMsgs(callback,msgKey,lastMsgTime,conti,fini,first,err) {
    if (selType == webim.SESSION_TYPE.GROUP) {
        alert('当前的聊天类型为群聊天，不能进行拉取好友历史消息操作');
        return;
    }
    var lastMsgTime = lastMsgTime;//第一次拉取好友历史消息时，必须传 0
    var msgKey =msgKey;
    var reqMsgCount = 15;
    var options = {
        ...
        console.log('准备',options)
    webim.getC2CHistoryMsgs(
        options,
        function (resp) {
            console.log(resp,'qiuqiu')
            if(lastMsgTime==0){
                first(1)
            }
            ...
            var newMsg;
            var msgreList = resp.MsgList.reverse();
            for (var j in msgreList) {//遍历新消息
                newMsg = resp.MsgList[j];
                console.log(j,resp.MsgList.length)
                if(j==resp.MsgList.length-1){
                    if(lastMsgTime==0){
                        callback(newMsg,1,1);
                    }else{
                        callback(newMsg,1,0);
                    ...
                    callback(newMsg,0,0);//处理新消息
                }

            }
            if(!complete){
                conti(resp.MsgKey,resp.LastMsgTime)
            }
...
function onMsgNotify(newMsgList, callback) {
    var newMsg;
    console.log("***777***",newMsgList)
    for (var j in newMsgList) {//遍历新消息
        newMsg = newMsgList[j];
        callback(handlderMsg(newMsg));//处理新消息
    }
}
        ...
        case webim.SESSION_TYPE.C2C://私聊消息
            switch (subType) {
                case webim.C2C_MSG_SUB_TYPE.COMMON://c2c普通消息
                 ...
                 console.log(msg.elems[0].content.text,'kkkkkk')

                    if(msg.elems[0].content.text&&msg.elems[0].content.text.indexOf('csyzwfelab20180425hhhdfq')>-1){
                        var tempObj = JSON.parse(msg.elems[0].content.text);
                        contentHtml = tempObj;
                    }else{
...
//sdk登录
function sdkLogin(userInfo, listeners, options, callback, callbackOptions) {
    //web sdk 登录
    webim.login(userInfo, listeners, options,
        function (identifierNick) {
            ...
            //identifierNick为登录用户昵称(没有设置时，为帐号)，无登录态时为空
            console.debug(identifierNick);
            webim.Log.info('webim登录成功',userInfo);
            console.log('webim登陆成功',userInfo);
            loginInfo = userInfo;
            setProfilePortrait({
                'ProfileItem': [{
        ...
        function (err) {
            console.log('webim登录失败',err.message);
...
function applyJoinBigGroup(groupId, callback, callbackOptions) {
    ...
    console.log("***applyJoinBigGroup***",groupId)
    // 做一些预处理
...
function onSendMsg(msg,typedata,nickname,callback) {
    ...
    if(nickname){
        loginInfo.identifierNick = nickname;
    }
    if (!loginInfo.identifier) {//未登录
        if (accountMode == 1) {//托管模式
            //将account_type保存到cookie中,有效期是1天
    ...
    if(typedata.myselToID){
        selToID=typedata.myselToID
    }
    if (!selToID) {
    ...
    var myselType = typedata.TYPE || selType;
    var myselToID = typedata.myselToID || selToID;
    var maxLen, errInfo;
    if (myselType == webim.SESSION_TYPE.GROUP) {
    ...
    if (msgLen > maxLen) {
        ...
        selSess = new webim.Session(myselType, myselToID, myselToID, selSessHeadUrl, Math.round(new Date().getTime() / 1000));
    ...
    webim.sendMsg(msg, function (resp) {
        if (myselType == webim.SESSION_TYPE.C2C) {//私聊时，在聊天窗口手动添加一条发的消息，群聊时，长轮询接口会返回自己发的消息
            showMsg(msg);
        }
        webim.Log.info("发消息成功");
        console.log("发消息成功");
        callback && callback(showMsg(msg));
 ...
 */
function sendCustomMsg(msg,typeData,nickname,callback) {
    if(typeData.myselToID){
        selToID=typeData.myselToID
    }
    selType = typeData.TYPE || selType;
    if (!loginInfo.identifier) {//未登录
        if (accountMode == 1) {//托管模式
            //将account_type保存到cookie中,有效期是1天
            webim.Tool.setCookie('accountType', loginInfo.accountType, 3600 * 24);
...
//退出大群
function quitBigGroup(roomID) {
    console.log("***注释掉判断语句，请注意quitBigGroup webim_handler.js:1082***",avChatRoomId,roomID);
    // if(!avChatRoomId){  // 先注释掉
    //     return false;
    // }
    var options = {
        'GroupId': avChatRoomId==0 ? roomID : avChatRoomId //群id
    };
    console.log("***===quitBigGroup===***",options);
    webim.quitBigGroup(
        options,
        function (resp) {
        ...
        function (err) {
            console.error("TTTTTTTTTT",err.ErrorInfo);
        }
    );
}
