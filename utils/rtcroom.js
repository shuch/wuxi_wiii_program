...
var noReport = ['pusher_heartbeat', 'get_room_list', 'create_room', 'delete_pusher','get_pushers','']; // logout
// 时间戳
var ts = {
	join_group_srart: 0,
	get_pushers_start: 0,
...
function request(options) {
	...
	var req_url=serverDomain + options.url + (options.params?('?' + formatParams(options.params) + '&'):'?') + 'userID=' + accountInfo.userID + (accountInfo.token?'&token=' + accountInfo.token:"");
	...
	requestTask[requestSeq++] = wx.request({
		...
		success: function(ret) {
			...
			if(ret.data.code) {
				...
				switch (options.url) {
					case 'get_push_url': {
						app.globalData.dataJson+=JSON.stringify({"get_pushers":code,"url":req_url});
						report.setReportData({ int64_tc_get_pushers: code});
						break;
					}
					case 'add_pusher': {
						app.globalData.dataJson+=JSON.stringify({"add_pusher":code,"url":req_url});
						report.setReportData({ int64_tc_add_pusher: code });
						break;
					}
				}
		...
		fail: function(ret) {
			...
			if (noReport.indexOf(options.url) == -1) {
				app.globalData.dataJson+=JSON.stringify({"请求失败上报：url":req_url});
...
function login(options) {
	...
	accountInfo.loginid = options.data.loginid;
	accountInfo.userSplitID = options.data.userSplitID;
	...
	accountInfo.accountType = options.data.accType;
  	accountInfo.userName = options.data.userName || options.data.loginid || accountInfo.userID;
	accountInfo.userAvatar = options.data.userAvatar || '123';
	accountInfo.gender = options.data.gender || '1';
	accountInfo.selToID = options.data.selToID;
	accountInfo.opt = options.data.opt;
	if(!options.data.userSig){
		console.log("***userSig***",options.data.userSig)
		wx.navigateTo({
...
function loginIM(options) {
	...
	webimhandler.init({
		...
		accountType: accountInfo.accountType,
		sdkAppID: accountInfo.sdkAppID,
		avChatRoomId: options.roomID || 0, // 群聊时的群组编号
		selType: webim.SESSION_TYPE.GROUP, // GROUP C2C
		selToID: accountInfo.selToID,//accountInfo.selToID || "6_18516561395_109",//options.roomID || 0, // 6_18516561395_109
		selSess: null //当前聊天会话
	});
	//当前用户身份
	var loginInfo = {
	...
	console.log("###loginIM rtcroom.js 321行### websdkappid=537048168 is web im sdk 版本 APPID",accountInfo)
	if (accountInfo.accountMode == 1) { //托管模式
		// webimhandler.sdkLogin(loginInfo, listeners, others, afterLoginIM, options);
		webimhandler.sdkLogin(loginInfo, listeners, others, afterLoginIM, options);
	} else { //独立模式
		//sdk登录
		// webimhandler.sdkLogin(loginInfo, listeners, others, afterLoginIM, options);
...
function sendRoomTextMsg(options) {
	//!options.data.data.replace(/^\s*|\s*$/g, '')
	if(!options) {
	...
	// webimhandler.onSendMsg("12345678",function(){
	// 	options.success && options.success();
	// });
	if(options.typedata.flag=="1"){ //发送视频邀请消息
		webimhandler.sendCustomMsg(options.data, options.typedata,'', function(){
			options.success && options.success();
		});
		// webimhandler.onSendMsg(options.data.msg, options.typedata, function(){
		// 	options.success && options.success();
		// });
	}
	else if(options.typedata.flag=="3"){ //发送普通消息
		webimhandler.onSendMsg(options.data.msg, options.typedata, function(){
			options.success && options.success();
		});
	}
...
function mergePushers() {
	...
	getPushers({
		...
		success: function(ret) {
			...
			// if(roomInfo.pushers.length>0&&ret.pushers.length>0&&roomInfo.pushers[0].userID==ret.pushers[0].userID){
			// 	console.log("******",roomInfo.pushers[0].userID,ret.pushers[0].userID)
			// 	event.onRoomClose({});
			// }
			...
			if(leavePushers.length) {
				console.log('退房:', JSON.stringify(leavePushers),"房间解散");
				event.onPusherQuit({
					pushers: leavePushers
				});
				// exitRoom({});
				event.onRoomClose({});//退房 处理app端被T出时,小程序退出逻辑,只有两个人在房间时才好用
			}
			// CGI拉取房间成员列表结束时间戳
...
function getPushers(options) {
	...
	request({
		...
		success: function(ret) {
			...
			// console.log('拉取所有主播信息成功',ret.data,typeof(ret.data.pushersMap));
			var returnPushers = [],isInRoom = 0;
			console.log('拉取所有主播信息成功',ret.data,typeof(ret.data.pushersMap));

			for (var index in ret.data.pushersMap){
			    // console.log(a[index]);
			    var item=ret.data.pushersMap[index];
			    // console.log("dsgasdg",item,accountInfo)
			    // 剔除自己
				if(item.userID != accountInfo.userID) {
					returnPushers.push(item);
				} else {
					isInRoom = 1;
				}
			}
			console.log('拉取所有主播信息成功',returnPushers);
...
function createRoom(options) {
	...
	roomInfo.roomInfo = options.data.roomInfo.roomName || '';
	roomInfo.roomID = options.data.roomInfo.roomID || '';
	roomInfo.pushers = [];
...
function proto_createRoom(options) {
	...
	// console.log("****proto_createRoom******",roomInfo)
	request({
		url: 'create_room',
		data: {
			userID: accountInfo.userID,
		...
		success: function(ret) {
			options.data.roomID = roomInfo.roomID
			...
			console.log("&&&&&&&proto_createRoom创建房间成功&&&&&",roomInfo.roomID,ret)
			if(roomInfo.isDestory) {
				console.log("***======roomInfo.isDestory by proto_createRoom rtcroom.js***")
				exitRoom({});
				return;
			}
			// 进入IM群聊天
...
function proto_joinPusher(options) {
	...
	request({
		...
		success: function(ret) {
			...
			heart = true;
			pusherHeartBeat(1);
			console.log("*****proto_joinPusher 928 rtcroom.js ****",roomInfo)
			callVideo(); 	//发起视频看房
			// sdMessage();	//通知顾问 隐藏处理极光推送 引起的app端问题
			// 进入IM群
			webimhandler.applyJoinBigGroup(roomInfo.roomID, afterJoinBigGroup, {
				success: function() {
					roomInfo.isPush = true;
					options.success && options.success({});
				},
				fail: function() {
					options.fail
...
// 发送消息通知
function sdMessage(){
	var name = app.globalData.single.nickname||app.globalData.phone||app.globalData.loginid||'小程序客户';
	var para={
      	houseId:config.houseId,
      	receiver:app.globalData.videoCustomer.id,
      	businessType:1,
      	workId:app.globalData.single.id,
      	title:'消息通知顾问',
      	code:'PUSH_003',
      	pushContent:'客户' + name + '发起视频看房请求',
      	receiverType:"adviser",
  	};
  	console.log("******sdMessage***",para);
	wx.request({
          ...
          console.log("***sdMessage-发送消息通知-success",res)
      	}
  })
}
function callVideo(){
	...
	console.log("*****callVideo-sendComment*****", JSON.stringify(data),accountInfo.selToID);
	// rtcroomCom.sendTextMsg("123456zjs");
	let tp = {
	    flag: "1", // 发送视频邀请消息 1 自定义 3 普通消息
	    myselToID: accountInfo.selToID
	}
	// {text:'用户已拒绝！',ext:JSON.stringify(data),data:''};
...
function enterRoom(options) {
	console.log("***enterRoom***",roomInfo.isPush)
	roomInfo.isDestory = false;
	// 进入房间时间戳
	report.setReportData({
...
function exitRoom(options) {
	...
	// 	})
	// }
	console.log("***webimhandler.quitBigGroup***",options)
	webimhandler.quitBigGroup(options.roomID);	// 退出IM大群
