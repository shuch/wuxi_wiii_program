        ...
        wx.request({
            ...
            success:function (res) {
                ...
                if(res.data.success){
                    ...
                    if(res.data.errorCode=='CUSTOMER.MOBILE.EXSISTED'){
                        wx.showToast({
                            title:'手机号已被绑定',
                            icon:'warn',
                        })
        ...
        webimhandler.getC2CHistoryMsgs(function(data,last,scroll){
            ...
            if(val==1){
                ...
                setTimeout(()=>{
                    ...
                    let text=app.globalData.imHelloWord?app.globalData.imHelloWord:'您好，我是项目的置业顾问，请问您想咨询什么问题？我在这里为您快速解答。';
                    console.log(text)
                    let data = {
                        "csyzwfelab20180425hhhdfq":"secretkey",
                        "param":{text:text},
                    ...
                    if(duringTime/1000/60/60>23){
                        ...
                        webimhandler.sendCustomMsg({text:text,ext:JSON.stringify(data),data:''},{TYPE:webim.SESSION_TYPE.C2C,myselToID:that.data.adviserId},app.globalData.identifier,function(msg){
                            that.receiveMsgs({content:text,type:false},false);
                            wx.setStorageSync('needRepeatTo'+that.data.adviserId,new Date().getTime())
                            wx.request({
    ...
    bindButtonTap: function(){
        ...
        if(this.data.isFirstSend){
            ...
            timeout = setInterval(()=>{
                ...
                if(second<1){
                    let text = '';
                    if(app.globalData.imRepeat){
                        text = app.globalData.imRepeat
                    }else{
